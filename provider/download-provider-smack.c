	...
	if (stat(dir, &dstate) == 0) {
		if ((dstate.st_uid == uid && (dstate.st_mode & (S_IRUSR | S_IWUSR)) == (S_IRUSR | S_IWUSR)) ||
			...
			if (smack_getlabel(dir, &dir_label, SMACK_LABEL_ACCESS) == 0 &&
				...
				TRACE_STRERROR("failed to uid:%d gid:%d smack_label[%s]", uid, gid, smack_label);
				if (dir_label)
					TRACE_ERROR("failed to get dir_label[%s]", dir_label);
			}
			free(dir_label);
		} else {
			TRACE_STRERROR("failed to uid:%d gid:%d smack_label[%s]", uid, gid, smack_label);
		...
		TRACE_STRERROR("failed to get state:%s", dir);
	}
	return ret;
}
